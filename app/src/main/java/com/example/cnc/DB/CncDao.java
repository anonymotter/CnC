...
public interface CncDao {
  ...
  @Query("SELECT * FROM " + CncDb.USER_TABLE)
  List<User> getAllUsers();

  @Query("SELECT * FROM " + CncDb.USER_TABLE + " WHERE username = :username")
  List<User> getUserByName(String username);

  @Query("SELECT * FROM " + CncDb.USER_TABLE + " WHERE username = :username AND password = :password")
  List<User> getUserByNameAndPassword(String username, String password);

  @Query("SELECT * FROM " + CncDb.USER_TABLE + " WHERE userId = :userId AND isDm = 1")
